
# TripAdvise – Project Roadmap  
A beginner-friendly task roadmap showing progress across Sprint 1, Sprint 2, and Sprint 3.

---

## ✅ Sprint 1 – Project Setup & Planning

### Setup Tasks
- [x] Create GitHub account  
- [x] Join KSU-IS GitHub organization  
- [x] Install GitHub Desktop  
- [x] Create your TripAdvise repository  
- [x] Add a README.md file describing the project idea  

### Project Idea & Research
- [x] Write project idea description (TripAdvise – Travel Budget + Trip Planner)  
- [x] Add at least ONE source link to a similar project (ex: https://github.com/Jswan-2005/GUI-Travel-Budget-Planner)  
- [x] Look at similar projects and evaluate what they do well  
- [x] Write notes on features you want to copy or improve  

### Basic Code Work
- [x] Start the very first Python file (a simple placeholder program)  
- [x] Upload the code to GitHub  
- [x] Make at least 2–3 commits explaining what you changed  
- [x] Update README.md to show your project plan  

---

## ✅ Sprint 2 – Start Coding & Tracking Tasks

### Build the First Working Version
- [ ] Create the “Trip Budget Calculator” basic Python script  
- [ ] Add inputs for flight, hotel, food, transportation, activities  
- [ ] Compute total trip cost  
- [ ] Compute average cost per day  
- [ ] Print results clearly  

### Improve Git/GitHub Workflow
- [ ] Make small commits (min. 6 significant commits)  
- [ ] Write meaningful commit messages  
- [ ] Push code frequently so the professor can see progress  
- [ ] Organize files inside your repo  

### Update Roadmap During Sprint
- [ ] Check off completed tasks  
- [ ] Add new tasks as needed  
- [ ] Mark tasks as “DONE” in this roadmap  
- [ ] Add any new features discovered while coding  

---

## ✅ Sprint 3 – Refining, Testing & Splash Screen

### Code Refinement
- [ ] Test all user inputs  
- [ ] Add error-handling for invalid numbers  
- [ ] Improve calculations  
- [ ] Improve display/output formatting  
- [ ] Add optional features (daily breakdown, tips, savings mode)  

### Create Sprint 3 PPT Slide
Your slide must include:
- [ ] List your name  
- [ ] Title of project: **TripAdvise**  
- [ ] One-sentence project purpose  
- [ ] Short tag line  
- [ ] 1–2 screenshots or simple visuals  

### GitHub Sprint 3 Requirements
- [ ] Upload your Sprint 3 slide to GitHub repo  
- [ ] Keep making commits and updates  
- [ ] Final cleanup before submission  
